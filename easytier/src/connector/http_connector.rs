...
impl HttpTunnelConnector {
    ...
    async fn handle_302_redirect(
        ...
        // 1: http(s)://easytier.cn/tcp/10.147.22.22:11010 (scheme is http, domain is ignored, path is splitted into proto type and addr)
        ...
        if url.scheme() == "http" || url.scheme() == "https" {
            ...
            self.redirect_type = HttpRedirectType::RedirectToPath;
...
mod tests {
    ...
    async fn run_http_redirect_server(port: u16, test_type: HttpRedirectType) -> Result<(), Error> {
        ...
        match test_type {
            HttpRedirectType::RedirectToPath => {
                let resp = "HTTP/1.1 301 Moved Permanently\r\nLocation: http://test.com/tcp/127.0.0.1:25888\r\n\r\n";
